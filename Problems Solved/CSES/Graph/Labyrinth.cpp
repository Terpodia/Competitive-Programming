...
void solve(ii start, ii end){
    bool vis[N][M]; mem(vis, false);
    queue<node> Q; Q.push({start, {-1,-1}, '!'});
    pair<ii, char> ans[N][M]; ans[0][0] = {make_pair(-1,-1),'!'};
    ...
    while(!Q.empty()){
        ...
        forn(i, 0, 4){
            ...
            if(inside(x2,y2) && !vis[x2][y2]){ Q.push({make_pair(x2,y2), p, c}); vis[x2][y2] = true; }
        }
    }
