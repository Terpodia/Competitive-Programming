...
void BFS(bool monster, pair<int,int>& end){
    while(!Q.empty()){
        ...
        if(!monster){
            ...
            if(c.pos.first == 0 || c.pos.first == n-1 || c.pos.second == 0 || c.pos.second == m-1){
                end = c.pos;
                return;
            }
        }
        ...
        forn(i, 0, 4){
            ...
            char dir;
            (i == 0)? dir = 'D' : (i == 1)? dir = 'U' : (i == 2)? dir = 'R' : dir = 'L';
...
int main(){
	...
	forn(i,0,n) forn(j, 0, m){
        cin >> a[i][j];
        if(a[i][j] == 'A') startpos = {i,j};
        else if(a[i][j] == 'M') Q.push({make_pair(i,j), make_pair(-1,-1), '!', 0});
	}
