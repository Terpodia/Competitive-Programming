...
class SegmentTree {
 ...
 public:
  ...
  void update(int j, int v) { update(1, 0, n - 1, j, v); }
...
int hld_query(int u, int l) {
  ...
  if (nxt[u] == nxt[l]) return st.rmq(in[G[l][0].second], in[p[u].second]);

  if (u != nxt[u])
    return max(st.rmq(in[G[nxt[u]][0].second], in[p[u].second]),
               hld_query(nxt[u], l));
