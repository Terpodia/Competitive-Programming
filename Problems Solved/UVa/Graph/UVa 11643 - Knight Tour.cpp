...
const int dr[8] = { 2, 2, 1, 1, -2, -2, -1, -1  }, dc[8] = { 1, -1, 2, -2, 1, -1, 2, -2  }, INF = 1e9;
...
int bfs(pair<int,int> s, pair<int,int> t) {
  ...
  while (!q.empty()) {
    ...
    for (int i=0; i<8; i++) {
      int vr = dr[i] + ur, vc = dc[i] + uc;
      if (vr < 0 || vc < 0 || vr >= n || vc >= n || curr[vr][vc] == it) continue;
...
void solve(int tc) {
  ...
  for (int i=0; i<k; i++) {
    for (int j=0; j<k; j++) {
      ...
      if (diff_r + diff_c >= 3) dist[i][j]  = dist_pre[diff_r][diff_c];
      else {
        it++; 
