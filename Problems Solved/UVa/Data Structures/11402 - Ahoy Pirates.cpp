    ...
    void propagate(int p, int L, int R) {
      if (lazy[p] != -1) {
        if (lazy[p] == -2) {
          st[p] = (R - L + 1) - st[p];
          if (L != R) {
            if (lazy[l(p)] == -1) lazy[l(p)] = lazy[p];
            else if (lazy[l(p)] == 1) lazy[l(p)] = 0;
            else if (lazy[l(p)] == 0) lazy[l(p)] = 1;
            else lazy[l(p)] = -1;

            if (lazy[r(p)] == -1) lazy[r(p)] = lazy[p];
            else if (lazy[r(p)] == 1) lazy[r(p)] = 0;
            else if (lazy[r(p)] == 0) lazy[r(p)] = 1;
            else lazy[r(p)] = -1;
          }
        }
        else {
...
int main() {
  ...
  FOR(_,1,TC+1) {
    ...
    while (Q--) {
      ...
      if (c == 'F') st.setValue(a, b, 1);
      else if (c == 'E') st.setValue(a, b, 0);
      else if (c == 'I') st.inverse(a, b);
      else ans.push_back(st.rsq(a, b));
    }
