...
vector<vector<int>> build_psum2D(int x) { 
    ...
    for(int i=1; i<=n; i++)
        for(int j=1; j<=n; j++)
            ret[i][j] = ret[i-1][j] + ret[i][j-1] - ret[i-1][j-1] + b[i-1][j-1];

    return ret;
}

int query(int r1, int c1, int r2, int c2, vector<vector<int>>& psum2D) { 
    return psum2D[r2][c2] - psum2D[r2][c1-1] - psum2D[r1-1][c2] + psum2D[r1-1][c1-1];
}
...
bool can(int x) { 
    ...
    for(int i=0; i+k-1 < n; i++) {
        for(int j=0; j+k-1 < n; j++) {
            if(query(i+1, j+1, i+k, j+k, psum2D) >= (k*k+1)/2) return true;
        }
    }
