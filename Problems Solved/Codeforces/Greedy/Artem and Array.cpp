...
ll solve() {
    ...
    for(int i=1; i<n-1; i++)
        if(a[adj[i][0]] >= a[i] && a[adj[i][1]] >= a[i]) 
            q.push(i);
    ...
    while(!q.empty()) {
        ...
        if(adj[i][0] > 0 && a[adj[adj[i][0]][0]] >= a[adj[i][0]] && a[adj[adj[i][0]][1]] >= a[adj[i][0]]) 
            q.push(adj[i][0]);

        if(adj[i][1] < n-1 && a[adj[adj[i][1]][0]] >= a[adj[i][1]] && a[adj[adj[i][1]][1]] >= a[adj[i][1]]) 
            q.push(adj[i][1]);
    ...
    while(true) {
        ...
        if(adj[curr][0] == 0) curr = adj[curr][1];
        else if(adj[curr][1] == n-1) curr = adj[curr][0];
        else {
            if(min(a[adj[adj[curr][0]][0]], a[adj[adj[curr][0]][1]]) >= min(a[adj[adj[curr][1]][0]], a[adj[adj[curr][1]][1]]))
                curr = adj[curr][0];
            else
                curr = adj[curr][1];
        }
