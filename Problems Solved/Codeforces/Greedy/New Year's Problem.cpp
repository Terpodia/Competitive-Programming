...
int solve() {
  ...
  FOR(i,0,n) FOR(j,1,m) {
    if (used[friends[i][j].second]) hi = max(hi, friends[i][j].first);
    else {
      if (t[friends[i][j].second][0].second != i) hi = max(hi, friends[i][j].first);
      else hi = max(hi, t[friends[i][j].second][1].first);
    }
  }
