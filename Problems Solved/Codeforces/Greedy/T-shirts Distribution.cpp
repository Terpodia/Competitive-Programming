...
void solve() {
    ...
    for(int i=0; i<n; i++) {
        if(a[i].second == 0) {
            if(shirts[a[i].first] == 0) can = false; 
            else {
                shirts[a[i].first]--;
                ans[i] = a[i].first;
            }
        ...
        else b.push_back(make_pair(a[i].first, i));
    }
    ...
    for(int i=0; i<sz(b); i++) {
        if(shirts[b[i].first] > 0) {
            ...
            if(shirts[b[i].first+1] == 0) can = false;
            else {
...
int main() {
    ...
    cin >> shirts[1] >> shirts[2] >> shirts[3] >> shirts[4] >> shirts[5] >> shirts[6] >> n;
