...
vector<int> merge(vector<int>& v1, vector<int>& v2) {
  ...
  while ((i != v1.end() || j != v2.end()) && sz(ret) <= 10) {
    if (i == v1.end()) {
      if (find(ret.begin(), ret.end(), *j) == ret.end()) ret.push_back(*j);
      j++;
    }
...
void DFS(int u, int p = -1, vector<int> s = vector<int>{}) {
  ...
  for (int i = 1; i < MAXLOG; i++) {
    if (anc[u][i - 1] != -1)
      ...
      anc_people[u][i] =
          merge(anc_people[u][i - 1], anc_people[anc[u][i - 1]][i - 1]);
  }

  for (int v : G[u])
    if (v != p) DFS(v, u, people[u]);
}
...
int main() {
  ...
  for (int i = 0; i < m; i++) {
    ...
    scanf("%d", &c);
    c--;
    people[c].push_back(i + 1);
    if (sz(people[c]) > 10)
      people[c].erase(max_element(people[c].begin(), people[c].end()));
  }

  for (int i = 0; i < n; i++) sort(people[i].begin(), people[i].end());
