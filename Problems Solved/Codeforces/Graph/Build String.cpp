...
void construct() {
  ...
  fore(i, 1, n + 1) {
    g.add_edge(0, cur, a[i - 1], 0);
    vector<int> cnt(26, 0);
    for (char c : s[i - 1]) {
      cnt[c - 'a']++;
    }
    forn(c, 26) { g.add_edge(cur, cur + c + 1, cnt[c], i); }
    forn(c, 26) {
      g.add_edge(cur + c + 1, dest + c + 1, min(cnt[c], cntT[c]), 0);
    }
    cur += 26 + 1;
  }
  forn(c, 26) { g.add_edge(dest + c + 1, dest, cntT[c], 0); }
}
...
int main() {
  ...
  cin.tie(0);
  cin >> t >> n;
  forn(i, n) cin >> s[i] >> a[i];

  construct();
  auto [flow, cost] = g.get_flow(0, dest);
  if (flow >= len(t))
    cout << cost << "\n";
  else
    cout << "-1\n";
