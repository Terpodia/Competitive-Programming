...
int main() {
    ...
    for(int i=0; i<m; i++) {
        ...
        else uset.insert(u), uset.insert(v);
    }

    vector<vll> dist(sz(uset), vll(n)); int j=0;

    for(auto i = uset.begin(); i != uset.end(); i++, j++) 
        dist[j] = dijkstra(*i);
