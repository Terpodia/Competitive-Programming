...
void solve(pi s, pi t) {
  ...
  vector<vector<vector<int>>> dist(n,
                                   vector<vector<int>>(m, vector<int>(5, INF)));

  Q.push_front(make_tuple(s.f, s.s, 4, -1));
  dist[s.f][s.s][4] = dist[s.f][s.s][3] = dist[s.f][s.s][2] =
      dist[s.f][s.s][1] = dist[s.f][s.s][0] = -1;

  while (!Q.empty()) {
    tuple<int, int, int, int> query = Q.front();
    Q.pop_front();
    ...
    forn(_, 0, 4) {
      ...
      if (z < dist[x][y][_]) {
        dist[x][y][_] = z;
        (z == cnt) ? Q.push_front(make_tuple(x, y, _, z))
                   : Q.push_back(make_tuple(x, y, _, z));
      }
    }
  }

  (dist[t.f][t.s][0] < 3 || dist[t.f][t.s][1] < 3 || dist[t.f][t.s][2] < 3 ||
   dist[t.f][t.s][3] < 3)
      ? cout << "YES\n"
      : cout << "NO\n";
}
