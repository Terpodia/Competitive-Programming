...
struct seg_tree{
    ...
    void build(ll array[], int l, int r, int x){
        if(l == r){
            if(array[l] > 0) nodes[x] = {array[l], array[l], array[l], array[l]};
            else nodes[x] = {array[l], 0, 0, 0};
            return;
        }
        int m = (l + r)/2;
        build(array, l, m, x*2);
    ...
    void update(int u, int v, int l, int r, int x){
        if(l == r){
            if(v > 0) nodes[x] = {v, v, v, v};
            else nodes[x] = {v, 0, 0, 0};
            return;
        }
 
        int m = (l + r)/2;
        if(u <= m) update(u, v, l, m, x*2);
        else update(u, v, m+1, r, x*2+1);
