...
int calc(int i=0, int cnt=0) {
    ...
    if(s[i] != '?') {
        ...
        int a = calc(i+1, cnt), b = calc(i+1, cnt+1), c = calc(i+1, cnt-1);

        if(a == 1 || b == 1 || c == 1) ret = 1;
        else ret = 0;
    }
...
string solve(int i, int cnt) {
    ...
    if(dp[i+1][x] == 1) ret.push_back('D');
    else if(dp[i+1][(x+1)%MAXK] == 1) { ret.push_back('W'); sum=1; }
    else { ret.push_back('L'); sum=-1; }
