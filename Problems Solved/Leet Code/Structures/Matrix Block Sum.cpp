...
{
    vector<vector<int>> pref(sz(mat) + 1, vector<int>(sz(mat[0]) + 1, 0)), ans(sz(mat), vector<int>(sz(mat[0]))); 
    ...
    for(int i=0; i<sz(mat); i++){
        for(int j=0; j<sz(mat[0]); j++){
            ...
            ans[i][j] = pref[d.f+1][d.s+1] - pref[d.f+1][s.s] - pref[s.f][d.s+1] + pref[s.f][s.s];
        }
    }
