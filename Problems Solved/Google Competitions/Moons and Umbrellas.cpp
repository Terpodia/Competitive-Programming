...
int solve(int i, int lastCharacter){
    ...
    if(c != -1){
        if(lastCharacter == 0 && c == 1) return dp[i][lastCharacter] = X + solve(i+1, c);
        return dp[i][lastCharacter] = Y + solve(i+1, c);
    }
    
    if(lastCharacter == 0) return dp[i][lastCharacter] = min(solve(i+1, 0), solve(i+1, 1) + X);
...
int main(){
    ...
    while(t--){
        ...
        int ans=0;
        if(lastCharacter != -1) ans = solve(1, lastCharacter);
