...
int main(){
    ...
    forn(i, 0, M){
        ...
        if(dsu.get(u) != dsu.get(v)){
            dsu.merge(u, v); ans = min(ans, w); 
            if(!ok[sites[u]] && dsu.get(u) == dsu.get(sites[u])) { cnt++; ok[sites[u]] = true; }
            if(!ok[u] && dsu.get(pos[u]) == dsu.get(u)) { cnt++; ok[u] = true; } 

            if(!ok[sites[v]] && dsu.get(v) == dsu.get(sites[v])) { cnt++; ok[sites[v]] = true; }
            if(!ok[v] && dsu.get(pos[v]) == dsu.get(v)) { cnt++; ok[v] = true; } 
        }
    }
