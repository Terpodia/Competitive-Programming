...
int main(){
    ...
    int N, M; pos f, b;
    fin >> N >> M;
    fin >> f.x >> f.y; fin >> b.x >> b.y;

    pos path1[N], path2[M];
    for(int i=0; i<N; i++){
        char c; fin >> c;
        if(c == 'N') path1[i] = {0, 1};
        else if(c == 'S') path1[i] = {0, -1};
        else if(c == 'E') path1[i] = {1, 0};
        else path1[i] = {-1, 0};
    }
    ...
    for(int i=0; i<M; i++){
        char c; fin >> c;
        if(c == 'N') path2[i] = {0, 1};
        else if(c == 'S') path2[i] = {0, -1};
        else if(c == 'E') path2[i] = {1, 0};
        else path2[i] = {-1, 0};
    }
    ...
    for(int i=1; i<=N; i++){
        ...
        for(int j=1; j<=M; j++){
            ...
            dp[i][j] = min(dp[i-1][j-1]+d, min(dp[i-1][j]+d, dp[i][j-1]+d));
        }
    }
