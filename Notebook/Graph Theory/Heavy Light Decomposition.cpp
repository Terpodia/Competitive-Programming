...
int main() {
  int n, q; scan(n, q);
  p = vi(n, -1), h = vi(n, 0), sz = vi(n, 0), val = vi(n), in = vi(n, 0), out = vi(n, 0), a = vi(n+7, 0), nxt = vi(n, 0);
  ...
  while (q--) {
    int type, u, v; scan(type, u, v);
    if (type == 1) update_query(u-1, v);
    else printf("%d ", path_query(u-1, v-1));
  }
