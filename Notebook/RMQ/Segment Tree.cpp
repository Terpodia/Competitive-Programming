...
struct SegmentTree {
    ...
    void update(int val, int pos, int lx, int rx, int u) {
        if(lx == rx) { st[u] = val; return; }
        int m = (lx + rx) / 2;

        if(pos <= m) update(val, pos, lx, m, u*2);
        else update(val, pos, m+1, rx, u*2+1);
    ...
    void update(int val, int pos) {
        update(val, pos, 0, sz(st)/4 - 1, 1);    
    }
};

int main() {
    int n, q; scanf("%d%d", &n, &q); // size of the array and number of queries
